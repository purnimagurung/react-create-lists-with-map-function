...
const JobPage = () => {
  ...
  const deleteJobMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries(['jobs']); // Invalidate the 'jobs' query to refetch the list
      navigate('/jobs');
    },
    onError: () => {
  ...
  if (error) return <p>Error loading job data</p>;
          ...
          <div className="grid grid-cols-1 md:grid-cols-70/30 w-full gap-6">
            ...
            <aside>
              ...
              <div className="bg-white p-6 rounded-lg shadow-md mt-6">
                ...
                <Link
                  to={`/edit-job/${job.id}`}
                  className="bg-sky-500/100 hover:bg-indigo-600 text-white text-center font-bold py-2 px-4 rounded-full w-full focus:outline-none focus:shadow-outline mt-4 block"
                >
                  Edit Job
                </Link>
                <button
